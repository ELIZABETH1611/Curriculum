...
class AirHockeyCurriculum(AirHockeySingle):
    ...
    def _modify_observation(self, obs):
        ...
        obs = np.concatenate([obs, [self.goal[0],self.goal[1],(self.env_info['table']['goal_width'])/2]])
        return obs
    ...
    def setup(self, state=None):
        ...
        if self.random_init:
            ...
            if 25>self.Idx>=0 and self.Idx!=self.last_idx:
                if self.Idx==0:
                    self.target_range = np.array([-0.35,0.35])
                    self.target_width = np.array([-0.35,0.35])
                if self.Idx%5==0 and self.Idx<11:
                    reduction_w=0.15/2.0
            ...
            elif  self.Idx>=25 and self.Idx!=self.last_idx:
