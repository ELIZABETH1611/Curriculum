...
class AirHockeyCurriculum(AirHockeySingle):
    ...
    def _create_info_dictionary(self, obs):
        ...
        if self.save_data:
            # constraints["puck_start_x"]=np.array(self.puck_pos_i[0])
            # constraints["puck_end_x"]=np.array(self.puck_pos_f[0])
            # constraints["mallet_end_x"]=np.array(self.ee_pos[0]) 
            # constraints["puck_start_y"]=np.array(self.puck_pos_i[1])
            # constraints["puck_end_y"]=np.array(self.puck_pos_f[1])
            # constraints["mallet_end_y"]=np.array(self.ee_pos[1])
            self.save_Data (self.puck_pos_i[0],self.puck_pos_i[1],self.puck_pos_f[0],self.puck_pos_f[1],self.ee_pos[0],self.ee_pos[1])

            # print(constraints["mallet_end"],constraints["puck_start"],constraints["puck_end"])
        return constraints
    
    def save_Data(self,x,y,x1,y1,x2,y2):
        # print("saving",self.dirPath)
        if not os.path.exists(self.dirPath +"value_agent_"+str(20)+'.txt'):
            with open(self.dirPath+"value_agent_"+str(20)+'.txt', 'a') as outfile:
