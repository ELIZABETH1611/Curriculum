...
class AirHockeyCurriculum(AirHockeySingle):
    ...
    #     if self.Ad_T:
    #         obs_low = np.concatenate([mdp_info.observation_space.low, [-0.6]])
    #         obs_high = np.concatenate([mdp_info.observation_space.high, [1.]])
    ...
    #     if self.Ad_T:
    #         obs = np.concatenate([obs, [self.goal[0]]])
    #         return obs
    #     else:
    #         return obs
    ...
    def set_puck(self, inc_x=0.0, cn=0.0):
        inc_y = inc_x/2
        cn_y = cn/2
        inc_r = inc_y
        if inc_x>0.40 :
        ...
        if self.kepp_training:
            ...
            if self.Value2<=self.split:
                self.hit_range = np.array([[-0.65, -0.65+inc_x], [-0.35, 0.35]])
            else:
                self.hit_range = np.array([[-0.65, -0.25], [-0.35, 0.35]])       
        else:
            # self.hit_range = np.array([[-0.65, -0.65+inc_x], [-0.05-inc_y, 0.05+inc_y]])   
                if self.Value2<=self.split:
                    self.hit_range = np.array([[-0.65, -0.65+inc_x], [-0.10, 0.10]])
                else:
                    self.hit_range = np.array([[-0.65, -0.25], [-0.10-cn_y, 0.10+cn_y]])
    ...
    def get_distribution(self,value=0):
        ...
        if self.kepp_training:
            ...
            self.goal     = np.array([0.974,0.0])
        else:
            self.goal     = np.array([initial_value+(value*Length_dist),0.0])
        self._model.site_pos[2]  = np.concatenate((self.goal,0.0), axis=None)
        self.set_puck(value*Length_dist, self.Value*Length_dist )
        print(self.goal)
        return self.goal
