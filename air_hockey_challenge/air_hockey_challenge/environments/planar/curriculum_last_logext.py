...
class AirHockeyCurriculum(AirHockeySingle):
    ...
    def _modify_mdp_info(self, mdp_info):
        ...
        obs_low = np.concatenate([mdp_info.observation_space.low, [-1,-1,-1,-1,-0.6,-0.4,-1,-1]])
        obs_high = np.concatenate([mdp_info.observation_space.high, [1,1,1,1,1.,0.4,1,1]])
    ...
    def _modify_observation(self, obs):
        ...
        obs = np.concatenate([obs, [np.sin(self.ee_pos[0]),np.cos(self.ee_pos[0]),np.sin(self.ee_pos[1]),np.cos(self.ee_pos[1]),self.goal[0],self.goal[1], self.sen_theta,self.cos_theta]])
        return obs    
