...
class AirHockeyCurriculum(AirHockeySingle):
    ...
    def setup(self, state=None):
        ...
        if self.random_init:
            ...
            Samples= (-self.target_range[0]+self.target_range[1])/(self.env_info['table']['goal_width']/2.0)
            target_y= np.random.choice(np.linspace(self.target_range[0],self.target_range[1],round(Samples)+1)[self.mask_inf:self.mask_sup])
    ...
    def get_distribution(self,value=0):
        ...
        if value>self.split:
            value = self.split
        length        = self.env_info['table']['length'] - 4 * (self.env_info['puck']['radius']+self.env_info['mallet']['radius']) -0.05
        Length_dist   = length/self.split
        initial_value = - self.env_info['table']['length'] / 2 + 4 * (self.env_info['puck']['radius']+self.env_info['mallet']['radius'])+0.05          
        if self.kepp_training:
