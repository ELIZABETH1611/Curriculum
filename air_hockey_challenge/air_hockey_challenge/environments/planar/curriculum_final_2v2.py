...
class AirHockeyCurriculum(AirHockeySingle):
    ...
    def __init__(self, gamma=0.99, horizon=120, moving_init=False, random_init=False, CL_velocity= False, viewer_params={}):
        ...
        self.Idx=-1
        self.width_target =0.35
        self.target_width = np.array([-0.35, 0.35])  # Target X Frame
        self.target_range = np.array([-0.35, 0.35])  # Target X Frame
        self.reduction=((np.linalg.norm(self.target_width[0]-self.target_width [1]))/2)-(self.env_info['table']['goal_width']/2)
    ...
    def _modify_mdp_info(self, mdp_info):
        ...
        obs_low = np.concatenate([mdp_info.observation_space.low, [-0.6,-0.4,-0.35]])
        obs_high = np.concatenate([mdp_info.observation_space.high, [1.,0.4,0.35]])
    ...
    def _modify_observation(self, obs):
        ...
        obs = np.concatenate([obs, [self.goal[0],self.goal[1],self.env_info['table']['goal_width']]])
        return obs
    ...
    def setup(self, state=None):
        ...
        if self.random_init:
            ...
            if self.n_task>self.Idx>=0 and self.Idx!=self.last_idx:
                if self.Idx==0:
                    self.target_range = np.array([-0.35,0.35])
                    self.target_width = np.array([-0.35,0.35])
                if self.Idx%8==0 :
                    if self.target_width[1]<=0.25:
                        self.target_width = np.array([-0.25/2, 0.25/2])  # Target X Frame
                    else:
                        reduction_w=0.07/2.5
                        self.target_width = np.array([self.target_width[0]+ reduction_w, self.target_width[1]-reduction_w])
                reduction=(self.reduction/self.n_task)*self.Idx
                self.target_range = np.array([-0.35- self.target_width[0]+reduction, 0.35-self.target_width[1]-reduction]) 
                self.last_idx =self.Idx  # Target X Frame
            elif  self.Idx>=self.n_task and self.Idx!=self.last_idx:
                ...
                if self.Idx>=0:
                    ...
                    self.target_range = np.array([0.0,0.0])
                    self.target_width = np.array([-0.70/2, 0.70/2])  # Target X Frame
    ...
    def get_distribution(self,value=0):
        ...
        if value>=(self.split-self.n_task): # After task #30 We will star to reduce the area of sampling of the target 
            self.Idx+=1
