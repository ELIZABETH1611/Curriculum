...
from curriculum_atacom_iiwa import AirHockeyCurriculumPosition   # Curriculum Environment
...
def experiment(env: str = '3dof-hit',
    ...
    if load:
        f = open('/home/mcg/Videos/3dof-hit/args.json')
        data = json.load(f)
        
        results_dir="/home/mcg/REMOTE/curriculum/air_hockey_TU/TEST_9/ATACOM/logs/ACL_1_2023-10-06_01-17-18/experiment_id___0/0/3dof-hit/agent-0-221.msh"
        agent_path = Path(results_dir)
        rl_agent = RlAgent.load(agent_path)
        
        atacom = build_ATACOM_Controller(mdp.env_info,data['slack_type'], data['slack_beta'],data['slack_tol'],**kwargs)
        agent = ATACOMAgent(mdp.env_info, data['double_integration'], rl_agent, atacom)
    else:
    ...
    for epoch in range(n_epochs):
        ...
        if achievement>=threshold:
            ...
            rl_agent._target_entropy=mdp.decay_target_entropy(value)  # Reduces entropy
            logger.log_agent(rl_agent,epoch)
            value+=1
            logger.log_best_agent(rl_agent, J)
            mdp.get_distribution(value,value_2)   # Set the next task 
            value_2=value
...
def compute_metrics(core, eval_params):
    ...
    # Atacom does not need normalization because it does it internally.
