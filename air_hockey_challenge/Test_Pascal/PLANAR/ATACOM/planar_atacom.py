...
from curriculum_planar_atacom import AirHockeyCurriculumPosition 
...
def experiment(env: str = '3dof-hit',
    ...
    kwargs['debug'] = True
    random_init= True   # True: Distance Logarithm for Curriculum
    mdp = AirHockeyCurriculumPosition(random_init=random_init)
    mdp.moving_init = False
    mdp.sparce = True     # True: Sparce Reward [-10,0,10]
    mdp.split  = 30       # Number task curriculum 30 for log_Curri 20 for veloci_Curri
    value = 0
    value_2 = 0
    mdp.rectangle_width = 0.2
    mdp.rectangle_height= 0.2
    mdp.start_x=-0.65
    mdp.start_y=-mdp.rectangle_width/2.0
    mdp.bottom=True
    ...
    if load:
        results_dir="/home/mcg/REMOTE/curriculum/air_hockey_TU/TEST_9/logs/CRL_3_2023-10-06_16-56-26/experiment_id___0/0/3dof-hit/agent-0-best.msh"
    ...
    # rl_agent.policy._mu_approximator.model.network.model[-1].bias[2].data-=1
    # rl_agent.policy._mu_approximator.model.network.model[-1].bias[1].data-=1
    # rl_agent.policy._mu_approximator.model.network.model[-1].bias[0].data+=1
    ...
    for epoch in range(n_epochs):
        ...
        #############
        rl_agent = core.agent.rl_agent
        logger.log_best_agent(rl_agent, J)
        ...
        mdp.test_distribution=True
        mdp.hit_range = np.array([[-0.65, -0.25], [-0.35, 0.35]])
        J_e, R_e, E_e, V_e, alpha_e, achievement_FD = compute_metrics(core, eval_params)
        logger.log_numpy(Success_rate_FD = achievement_FD,J_FD = J_e,R_FD = R_e,E_FD = E_e)
        logger.log_numpy(G_P_FD = mdp.goal) 
        mdp.test_distribution = False
        print(value,achievement,achievement_FD)
        ##############EVALUATION TARGET DISTRIBUTION #########

        if achievement>=threshold:
            logger.log_agent(rl_agent,epoch)
            logger.log_best_agent(rl_agent, J)
            if value>4:
                mdp.moving_init=False
                mdp.original_position=True

            # ##############EVALUATION TARGET DISTRIBUTION #########
            # mdp.test_distribution=True
            # J_e, R_e, E_e, V_e, alpha_e, achievement_e = compute_metrics(core, eval_params)
            # logger.log_numpy(Success_rate_fd=achievement,J_fd=J_e,R_fd=R_e,E_fd=E_e)
            # logger.log_numpy(G_P_fd=mdp.goal ) 
            # mdp.test_distribution=False
            # ##############EVALUATION TARGET DISTRIBUTION #########
            
            mdp.increase_size(value)
